 ...
 * Includes tests for add, scale, dotProduct, crossProduct, length, normalize, etc.
 * Each test follows EP and BVA principles.
 * 
...
class VectorTests {
	...
	void testVectorDoubleDoubleDouble() {
	    ...
	    assertDoesNotThrow(() -> new Vector(1, 2, 3),
	        "Constructor with coordinates failed on valid vector");

	    // TC02: Valid vector with negative components (-1, -2, -3)
	    assertDoesNotThrow(() -> new Vector(-1, -2, -3),
	        "Constructor with coordinates failed on negative vector");
	...
	void testVectorDouble3() {
	    ...
	    assertDoesNotThrow(() -> new Vector(new Double3(1, 2, 3)),
	        "Constructor with Double3 failed on valid vector");
	 ...
	 * This test checks vector addition and throws exception when resulting in a zero vector
	 */
	@Test
	void testAddVector() {
		 // ============ Equivalence Partitions Tests ==============
	    ...
	    assertEquals(
	        new Vector(2, 3, 4),
	        new Vector(1, 1, 1).add(new Vector(1, 2, 3)),
	 ...
	 * This test checks scaling a vector by different scalar values,
     * and ensures scaling to zero vector is not allowed.
	 */
	@Test
	void testScale() {
		  // ============ Equivalence Partitions Tests ==============
	    ...
	    assertEquals(
	        ...
	        "scale() failed on positive scalar");

	    // TC02: Scale by negative scalar → should reverse direction
	    assertEquals(
	        new Vector(-1, -2, -3),
	        new Vector(1, 2, 3).scale(-1),
	        "scale() failed on negative scalar");
	 ...
	 * This test checks scalar (dot) product results between vectors.
	 */
	@Test
	void testDotProduct() {
		 // ============ Equivalence Partitions Tests ==============
	    ...
	    assertEquals(
	        14,
	        new Vector(1, 2, 3).dotProduct(new Vector(1, 2, 3)),
	        "dotProduct() failed on equal vectors");
	 ...
	 * This test checks that the result is orthogonal to both vectors,
     * and that an exception is thrown when vectors are parallel.
	 */
	@Test
	void testCrossProduct() {
	    ...
	    assertThrows(IllegalArgumentException.class,
	        () -> v1.crossProduct(new Vector(2, 4, 6)), // parallel to v1
	 ...
	 * This test checks the squared length of various vectors.
	 */
	@Test
	void testLengthSquared() {
	    ...
	    assertEquals(
	        ...
	        "lengthSquared() failed on positive vector");

	    // TC02: V = (-1, -2, -3) → same result: 1 + 4 + 9 = 14
	    assertEquals(
	        14,
	        new Vector(-1, -2, -3).lengthSquared(),
	        "lengthSquared() failed on negative vector");
	}

	/**
	 * Test method for {@link primitives.Vector#length()}.
	 * This test checks the actual length (magnitude) of a vector.
	 */
	@Test
	void testLength() {
		// TC01: V = (1, 2, 3) → sqrt(14)
	    assertEquals(
	        Math.sqrt(14),
	        new Vector(1, 2, 3).length(),
	        1e-10,
	        "length() failed on regular vector");
	    ...
	    assertEquals(
	        ...
	        1e-10,
	        "length() failed on vector with known length");
	}

	/**
	 * Test method for {@link primitives.Vector#normalize()}.
	 * This test checks that the result is a unit vector with the same direction
	 */
	@Test
	void testNormalize() {
	    ...
	    assertEquals(
	        ...
	        normalized.length(),
	        1e-10,
	        "normalize() did not produce unit vector");

	    // TC02: Direction should remain the same (dotProduct > 0)
	    assertTrue(
	        original.dotProduct(normalized) > 0,
	        "normalize() changed the direction of the vector");
	}
